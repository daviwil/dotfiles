...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Set the mode line font and size
  (set-face-attribute 'mode-line nil :font "Fira Mono" :height 120)
...
#+BEGIN_SRC emacs-lisp
  ...
  (unless (eq exwm--connection nil)
    (add-hook 'exwm-init-hook
      (lambda ()
        (exwm/run-in-background "compton --config ~/.dotfiles/emacs/compton.conf")
        (exwm/run-in-background "nitrogen --restore")
        (exwm/run-in-background "xfsettingsd")
        (exwm/run-in-background "xfce4-power-manager")
