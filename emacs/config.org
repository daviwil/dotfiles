...
#+BEGIN_SRC emacs-lisp
  ...
  (defun dw/run-xmodmap ()
    (interactive)
    (start-process-shell-command "xmodmap" nil "xmodmap ~/.dotfiles/i3/Xmodmap"))

  (defun dw/update-wallpapers ()
    (interactive)
    (start-process-shell-command "feh" nil "feh --bg-scale ~/Pictures/Backgrounds/mountains-1412683.jpg"))

  (defun dw/update-screen-layout ()
    (interactive)
    (let ((layout-script (pcase (system-name)
                            ("phantom" "~/.dotfiles/screen-layouts/x1e/docked.sh")
                            ("zerocool" "~/.dotfiles/screen-layouts/x1/docked.sh")
                            ("davinci" "~/.dotfiles/screen-layouts/t480s/docked.sh"))))
