...
First, run the following commands to unblock the wifi card and determine its device name:
...
Now run the following command to connect to your wifi, using the device name you received from =ifconfig -a=.  In my case it's =wlp4s0= so I run the command like so:
...
Since we're installing on a ThinkPad with UEFI, follow the [[https://guix.gnu.org/manual/en/guix.html#Disk-Partitioning][instructions in the Guix manual]] for disk partitioning.
...
Now your EFI and encrypted root filesystems are mounted so you can proceed with system installation.
...
To figure out the UUID of your encrypted root partition, you can use the following command:
